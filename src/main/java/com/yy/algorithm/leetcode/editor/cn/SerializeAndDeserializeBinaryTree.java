...
public class SerializeAndDeserializeBinaryTree {
    ...
    public class Codec {
        ...
        public TreeNode deserialize(String data) {
            ...
            return dfsDeserialize(datas);
        }

        private TreeNode dfsDeserialize(List<String> datas) {
            if (datas.isEmpty()) {
                return null;
            }
            String s = datas.remove(0);
            if (s.equals("NULL")) {
                return null;
            } else {
                TreeNode root = new TreeNode(Integer.parseInt(s));
                root.left = dfsDeserialize(datas);
                root.right = dfsDeserialize(datas);
                return root;
            }
        }
    }
